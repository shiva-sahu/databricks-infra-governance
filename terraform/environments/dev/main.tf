...
# This file is the source of truth for what exists in dev.
# ─────────────────────────────────────────────────────────────────────────────
...
terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "databricks-governance/dev/terraform.tfstate"
  }
}
...
module "workspace" {
  ...
  # Dev: no IP restriction — engineers access from anywhere
  allowed_ip_ranges      = []
...
module "unity_catalog" {
  ...
  adls_container       = "unitycatalog-dev"
  ...
  catalogs = {
    ecommerce = {
      team                = "ecommerce"
      domain              = "ecommerce"
      comment             = "E-commerce domain: orders, products, customers. Dev environment."
      ...
      grants = [
        ...
        # NOTE: dev_team gets read-only even in dev — good habit mirrors prod behaviour
        {
          principal  = "dev_team"
          privileges = ["USE_CATALOG", "SELECT"]
        }
    ...
    finance = {
      team                = "finance"
      domain              = "finance"
      comment             = "Finance domain: GL, AP, AR, FX. Dev environment."
...
module "cluster_policies" {
  ...
  # Dev: slightly more permissive node types for experimentation
  approved_node_types = [
    "Standard_DS3_v2",
    "Standard_DS4_v2",
    "Standard_DS5_v2",
  ...
  interactive_policy_groups = ["data_analysts", "data_engineers", "data_scientists", "dev_team"]
  job_policy_groups         = ["data_engineers", "dev_team"]
  ml_policy_groups          = ["data_scientists", "dev_team"]
  high_memory_policy_groups = ["data_engineers", "data_scientists", "dev_team"]
  single_user_policy_groups = ["data_engineers", "data_analysts", "data_scientists", "dev_team"]
}
...
module "rbac" {
  ...
  workspace_roles = [
    ...
    { group_name = "dev_team",                  can_create_clusters = true,  can_create_instance_pools = false, sql_access = true  },
  ]
